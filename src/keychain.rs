...
extern "C" {
    ...
    fn SecKeyGeneratePair(
        ...
        publicKey: *mut CFTypeRef,
        privateKey: *mut CFTypeRef,
    ) -> OSStatus;
...
impl CFDict {
    ...
    pub fn add_cfdata(mut self, key: CFStringRef, label: CFData) -> Self {
        unsafe {
            self.params
                .push((CFString::wrap_under_get_rule(key), label.as_CFType()));
        }
        self
    }
    ...
    pub fn add_cfdict(mut self, key: CFStringRef, dict: CFDictionary) -> Self {
        unsafe {
            self.params
                .push((CFString::wrap_under_get_rule(key), dict.as_CFType()));
        }
        self
    }
    ...
    pub fn add_boolean(mut self, key: CFStringRef, boolean: bool) -> Self {
        unsafe {
            ...
            self.params
                .push((CFString::wrap_under_get_rule(key), boolean));
        }
        self
    }
...
impl Keychain {
    ...
    fn get_pubkey_from_cfdictionary(key: CFDictionary) -> PubKey {
        ...
        unsafe {
            ...
            key_data = key
                .find(kSecValueRef as *const _)
                .map(|key_ref| {
                    CFDictionary::wrap_under_get_rule(
                        SecKeyCopyAttributes(key_ref) as CFDictionaryRef
                    )
                    .find(kSecValueData as *const _)
                    .map(|keydata| CFData::wrap_under_get_rule(keydata as *const _).to_vec())
                    .unwrap_or_else(|| Vec::new())
                })
                .unwrap();
        }
    ...
    pub fn generate_keypair(label: String) -> Result<(), &'static str> {
        unsafe {
            ...
            let status = SecKeyGeneratePair(
                gen_pair_dict.as_concrete_TypeRef(),
                &mut public_key_ref,
                &mut private_key_ref,
            );
